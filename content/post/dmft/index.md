...
What did we gain from this procedure? DMFT has allowed us to reduce the intractable integral
$$ \int dx \: e^{S_0(x)+S_V(x)} $$
to an implicit equation
$$ f^* = \langle f(x) \rangle_{f^*,\tilde f^*} $$
that needs to be solved by interative methods. Generally speaking, this is an easier problem than the integration. 

So far, the agreement between our DMFT approach and the exact solution is not particularly overwhelming. We didn't have any good justification to invoke the saddle-point approximation. 
Typically however, statistical physics is concerned with systems of many components, so that many observables of interest can be expected to concentrate. To see how we can benefit from this, let us slightly alter the calculation above by taking $ \bm{x}\in \mathbb{R}^N $ to be vector-valued. 



If the Free Theory decouples, i.e. $S_0(\bm{x})=\Pi_i^N S_0(x_i)$, we can rewrite
$$
\begin{aligned}
&\int df d\tilde f \:e^{f + i\tilde f f} \: \int d\bm{x} \,e^{S_0(\bm{x}) - i\bm{\tilde f} f(\bm{x})} \\
=& \int df d\tilde f \:e^{f + i\tilde f f} \: \Pi_i^N \int dx_i \,e^{S_0(x_i) - i\tilde f_i f(x_i)} \\
=& \int df d\tilde f \:e^{f + i\tilde f f} \: \left(\int dx \,e^{S_0(x) - i\tilde f f(x)}\right)^N \\
=&\int df d\tilde f \:e^{f + i\tilde f f} \:  e^{N\ln Z_{\text{eff}}(f, \tilde f)}
\end{aligned}
$$

Crucially, the $dfd\tilde f$ can now be expected to be concentrated at the point where the exponent is stationary, as all other contributions will be surpressed, given that $\ln Z_{\text{eff}}=\mathcal{O}(1)$. The plot below shows the saddle-point approximation for a theory of $$N=3$$. 
