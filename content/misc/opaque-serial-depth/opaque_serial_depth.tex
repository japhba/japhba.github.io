...
\begin{tikzpicture}[
  ...
  vedge/.style={->, draw=black!70, thick},                 % attention: same position (vertical)
  vatt/.style ={->, draw=black!28, thin},                  % attention: earlier positions
  hrec/.style ={->, draw=orange!88!black, very thick},     % recurrent step (the extra op)
...
\begin{scope}[shift={(\S,0)}]
  ...
  % additional horizontal (recurrent) operation
  \foreach \l in {1,...,\L}{ \foreach \t in {2,...,\T}{ \pgfmathtruncatemacro{\tm}{\t-1}
    \draw[hrec] (b\l\tm) -- (b\l\t);
  }}
\end{scope}
...
\node[ptitle,align=center] at (\S+2.5,4.4)
   {(b)\; attention $+$ horizontal recurrent step\\[2pt]\footnotesize opaque serial depth $\;\sim L+T$};
...
\node[anchor=west,align=left,font=\footnotesize] at (0.3,-1.7)
 {\textcolor{black!70}{$\rightarrow$}~attention (same position)\quad
  \textcolor{black!40}{$\rightarrow$}~attention (earlier positions)\quad
  \textcolor{orange!88!black}{$\boldsymbol\rightarrow$}~recurrent step (extra op)\quad
